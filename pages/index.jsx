...
const PROVIDERS = [
  { id:'claude', name:'Claude', logo:'🟠', placeholder:'sk-ant-api03-...', hint:'console.anthropic.com', hintUrl:'https://console.anthropic.com', model:'claude-sonnet-4-6' },
  { id:'openai', name:'OpenAI', logo:'⚫', placeholder:'sk-proj-...', hint:'platform.openai.com/api-keys', hintUrl:'https://platform.openai.com/api-keys', model:'gpt-4o' },
  { id:'gemini', name:'Gemini', logo:'🔵', placeholder:'AIzaSy...', hint:'aistudio.google.com/apikey', hintUrl:'https://aistudio.google.com/apikey', model:'gemini-1.5-flash' },
];
...
export default function App() {
  ...
  const saveDraft = (d,dt,n) => localStorage.setItem('trip_draft', JSON.stringify({dest:d,dates:dt,notes:n}));
  ...
  const generate = async () => {
    ...
    try {
      ...
      localStorage.removeItem('trip_checked'); localStorage.removeItem('trip_notes');
      setStep('plan');
  ...
  const toggleCheck = (key) => {
    ...
    setChecked(next); localStorage.setItem('trip_checked', JSON.stringify(next));
  };
  const saveNote = (key,val) => {
    const next = {...noteMap, [key]: val};
    setNoteMap(next); localStorage.setItem('trip_notes', JSON.stringify(next));
  };
  ...
  if (step === 'key') return (
    <div style={S.app}><div style={S.center}><div style={S.card}>
      ...
      <div style={{display:'flex',gap:'8px',marginBottom:'16px'}}>
        {PROVIDERS.map(p => (
          <button key={p.id} onClick={() => { setProvider(p.id); setApiKey(''); }}
            style={{flex:1,padding:'10px 8px',borderRadius:'10px',border:'2px solid '+(provider===p.id?'#1e293b':'#e2e8f0'),background:provider===p.id?'#1e293b':'white',color:provider===p.id?'white':'#64748b',cursor:'pointer',fontSize:'13px',fontWeight:'600',transition:'all .15s'}}>
            <div style={{fontSize:'18px',marginBottom:'2px'}}>{p.logo}</div>{p.name}
          </button>
        ))}
      </div>
      <label style={S.label}>{prov.name} API Key</label>
      <input style={S.input} type="password" placeholder={prov.placeholder} value={apiKey}
        onChange={e => setApiKey(e.target.value)}
        onKeyDown={e => { if(e.key==='Enter'&&apiKey){ localStorage.setItem('trip_api_key',apiKey); localStorage.setItem('trip_provider',provider); setStep('draft'); }}}/>
      <button style={{...S.btn, opacity:apiKey?1:0.5}} disabled={!apiKey}
        onClick={() => { localStorage.setItem('trip_api_key',apiKey); localStorage.setItem('trip_provider',provider); setStep('draft'); }}>
        เริ่มใช้งาน →
      </button>
      <p style={{textAlign:'center',fontSize:'12px',color:'#94a3b8',marginTop:'14px'}}>
        ขอ key ได้ที่ <a href={prov.hintUrl} target="_blank" rel="noreferrer" style={{color:'#3b82f6'}}>{prov.hint}</a>
      </p>
    </div></div></div>
  );
    ...
    <div style={S.app}><div style={{maxWidth:'600px',margin:'0 auto',padding:'16px'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
        ...
        <div style={{display:'flex',gap:'8px'}}>
          ...
          <button style={S.btnGhost} onClick={() => { localStorage.removeItem('trip_api_key'); localStorage.removeItem('trip_provider'); setStep('key'); }}>🔑 เปลี่ยน Key</button>
        </div>
      </div>
      <div style={{background:'white',borderRadius:'16px',padding:'20px',boxShadow:'0 2px 8px rgba(0,0,0,0.06)',marginBottom:'12px'}}>
        ...
        <div style={{fontSize:'12px',color:'#94a3b8',marginBottom:'8px'}}>พิมพ์ทุกอย่างที่รู้ ไม่ต้องเป็นระเบียบ — ร้านอาหาร, สถานที่, คำแนะนำจากเพื่อน</div>
      ...
      <div style={{fontSize:'12px',color:'#94a3b8',textAlign:'center',marginTop:'8px'}}>
        {prov.logo} ใช้ {prov.name} API ของคุณ · ประมาณ $0.01 ต่อครั้ง
      </div>
    </div></div>
  );
        ...
        <div style={{display:'flex',gap:'8px',padding:'10px 12px',overflowX:'auto',background:'white',borderBottom:'1px solid #e2e8f0'}}>
          {plan.days.map((d,i) => (
            <button key={i} onClick={() => setActiveDay(i)}
              ...
              <div>วัน {d.day}</div><div style={{fontSize:'10px',opacity:.8}}>{d.date}</div>
            </button>
          ))}
        </div>
